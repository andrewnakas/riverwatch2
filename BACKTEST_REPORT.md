...
- **True on-station holdout for `nwm`/`nwm_residual` in the build** (§4.1): score each station's archived issued forecasts against its own recent truth at build time. Needs the `nwm-archive` branch (or a compact extract) available to CI builds. Until then the UI number is an honest *estimate* (nwm hindcast MAE × measured holdout ratio), no longer a fabrication — but it's still a formula.
